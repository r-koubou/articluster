...
public class StreamReaderTest
{
    ...
    {
        using var reader = new TextStreamContentReader( File.OpenRead( Path.Combine( TestUtility.TestDataDirectoryRoot, "reader-text.txt" ) ) );
    ...
    {
        using var reader = new BinaryStreamContentReader( File.OpenRead( Path.Combine( TestUtility.TestDataDirectoryRoot, "reader-binary.bin" ) ) );
        var content = await reader.ReadAllAsync( CancellationToken.None );
