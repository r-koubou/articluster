...
using ArtiCluster.Features.Logic.ArticulationSetDefinitions.Infrastructures;
using ArtiCluster.Features.Logic.ArticulationSetDefinitions.UseCases;
using ArtiCluster.Shared.Domain.UniversalDefinitions.Model;
using ArtiCluster.Shared.IO.Abstractions;
using ArtiCluster.Shared.IO.Local;

using GatewayReason = ArtiCluster.Features.Logic.ArticulationSetDefinitions.Gateways.ExportReason;
...
public sealed class LogicDefinitionFacade : ILogicDefinitionFacade
{
    ...
    public async Task<Result<Unit, ExportReason>> ExportAsync( string exportDirectory, UniversalDefinition source, CancellationToken cancellationToken = default )
    {
        ...
        var outputPath = Path.Combine( outputDirectory, $"{source.ProductName.Value}.plist" );
