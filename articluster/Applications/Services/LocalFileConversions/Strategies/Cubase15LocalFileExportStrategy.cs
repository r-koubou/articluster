...
public sealed class Cubase15LocalFileExportStrategy : ILocalFileExportStrategy<UniversalDefinition>
{
    public string GetOutputDirectory( string baseDirectory, UniversalDefinition source )
        => Path.Combine( baseDirectory, "Cubase15", source.ManufacturerName.Value, source.ProductName.Value );
