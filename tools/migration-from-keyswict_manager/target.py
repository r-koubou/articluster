...
class Coordinate:
    ...
    product_name: str
    patch_name: str
    articluations: List[Articulation]
    description: str
    extra: Optional[Dict[str, Any]]

    def __init__(self, articluations: List[Articulation], author: str, description: str, extra: Optional[Dict[str, Any]], id: UUID, manufacturer_name: str, patch_name: str, product_name: str) -> None:
    ...
    def from_dict(obj: Any) -> 'Coordinate':
        assert isinstance(obj, dict)
        articluations = from_list(Articulation.from_dict, obj.get("Articulations"))
    ...
    def to_dict(self) -> dict:
        ...
        result["Articulations"] = from_list(lambda x: to_class(Articulation, x), self.articluations)
        if self.extra is not None:
            result["Extra"] = from_union([lambda x: from_dict(lambda x: x, x), from_none], self.extra)
        return result
